        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-700 text-white p-4 flex items-center justify-between relative">
          <div className="flex items-center">
            ...
            <div className="relative">
              <button onClick={() => setOpen(!open)} className="flex items-center gap-2 bg-blue-800 hover:bg-blue-900 px-3 py-1.5 rounded-lg text-sm transition-colors">
                ...
                <div className="absolute top-full left-0 mt-2 w-48 rounded-lg bg-white shadow-lg border border-gray-200 z-50 overflow-hidden">
                  <ul className="py-1">
                    {languages.filter(l => l !== language).map(l => (
                      <li key={l} onClick={() => handleSelect(l)} className="px-4 py-2.5 hover:bg-blue-50 cursor-pointer text-gray-800 text-sm">{l}</li>
                    ))}
                  </ul>
                </div>
              )}
          ...
          <div className="text-sm bg-blue-800 px-3 py-1 rounded-full">{t.version}</div>
        </div>
          ...
          <div className="flex-1 p-4 overflow-y-auto flex flex-col gap-4">
            ...
            {messages.map((message, i) => (
              <div key={i} className={`flex items-start gap-3 ${message.isUser ? "justify-end" : "justify-start"}`}>
                {!message.isUser && <div className="w-8 h-8 rounded-full bg-gradient-to-r from-blue-500 to-indigo-600 flex items-center justify-center flex-shrink-0"><i className="ri-robot-line text-white text-sm"></i></div>}
                <div className={`rounded-2xl p-4 max-w-[80%] ${message.isUser ? "bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-br-none" : "bg-white shadow-sm rounded-tl-none"}`}>
                  <p className="break-words">{message.text}</p>
                </div>
                {message.isUser && <div className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center flex-shrink-0"><i className="ri-user-line text-gray-700 text-sm"></i></div>}
              </div>
            ))}

            {isLoading && (
              <div className="flex items-start gap-3">
                <div className="w-8 h-8 rounded-full bg-gradient-to-r from-blue-500 to-indigo-600 flex items-center justify-center flex-shrink-0"><i className="ri-robot-line text-white text-sm"></i></div>
                <div className="bg-white rounded-2xl rounded-tl-none p-4 shadow-sm max-w-[80%]">
